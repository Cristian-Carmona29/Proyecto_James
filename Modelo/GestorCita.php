...
class GestorCita
{
    ...
    public function editarCita($cita_id, $nueva_fecha, $nueva_hora)
    {
        $conexion = new Conexion();
        $conexion->abrir();
        $sqlVerificar = "SELECT * FROM citas WHERE CitNumero = $cita_id";
        $conexion->consulta($sqlVerificar);

        if ($conexion->obtenerFilasAfectadas() > 0) {
            $sql = "UPDATE citas SET CitFecha = '$nueva_fecha', CitHora = '$nueva_hora' WHERE CitNumero = $cita_id";
            $conexion->consulta($sql);
            $filasAfectadas = $conexion->obtenerFilasAfectadas();
        } else {
            $filasAfectadas = 0;
